...
export class BOReport implements PipeTransform  {
  jwt: string;
  decodedJwt: string;
  response: string;
  api: string;
  ...
  private error:Error;
  private users: any[];  
  private laboral_bases: Array<string>;  
  private selected_filter:string;
  private transactionPercentage:double = 0;
  private completeTransactions:int= 0;
  private totalTransactions:int= 0;
  private totalUsers:int= 0;
  private _disabledV:string = '0';
  private disabled:boolean = false;

  constructor(public router: Router, public http: Http, public authHttp: AuthHttp) {      
	
    this.selected_filter = "Elegir Base";
  ...
  setStatistics(){
    ...
    this.setTransactionPercentage();
    this.completeTransactions = this.users.length - this.inactives.length;
...
setTransactionPercentage(){
  debugger;
  if(this.inactives !== undefined && this.users !== undefined){
    
    if(this.inactives.length > 0){
      this.transactionPercentage = ((this.users.length - this.inactives.length)* 100)/ this.users.length;  
    }
    else{
     this.transactionPercentage = 100; 
    }    
...
exportTransactionsToCsv(data){
  ...
  data.forEach((tran,index)=>{
    formattedData.push(
      ...
      date:tran.date,
      file:tran.user.file,
      laboral_base:tran.user.laboral_base,
      product:tran.product.title}
      );

  });

  let actualDate = this.transform(Date.now());
  new Angular2Csv(formattedData, 'bunge'+ actualDate);

}



exportInactivesToCsv(data){
